...
function KpiCard({ label, value, sub, icon: I, tone = "brand" }: {
  ...
  return (
    <div className="rounded-xl sm:rounded-2xl bg-white border border-[--color-hairline] px-2 sm:px-4 py-1.5 sm:py-3 shadow-sm flex items-center gap-1.5 sm:gap-3 min-w-0">
      <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl grid place-items-center shrink-0 ${toneMap[tone]}`}><I size={13} /></div>
      <div className="min-w-0 flex-1 leading-tight">
        <div className="text-[9px] sm:text-[10.5px] uppercase tracking-wider font-semibold text-[--color-muted] truncate">{label}</div>
        <div className="text-[13px] sm:text-[17px] font-bold text-[--color-ink] leading-tight truncate">{value}</div>
        {sub && <div className="text-[9px] sm:text-[10.5px] text-[--color-muted] truncate">{sub}</div>}
      </div>
    </div>
  );
}
    ...
    <div className="flex h-[calc(100vh-56px)] bg-[--color-canvas] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="shrink-0 bg-white border-b border-[--color-hairline]">
          ...
          {/* KPI strip */}
          <div className="px-3 sm:px-6 pb-2.5 sm:pb-3 grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-6 gap-1.5 sm:gap-2">
