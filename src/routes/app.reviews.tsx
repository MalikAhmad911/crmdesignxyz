  ...
  return (
    <div className="px-3 sm:px-6 lg:px-8 py-4 sm:py-6 max-w-[1400px] mx-auto">
      <PageHeader
        ...
        actions={
          <Btn variant="gradient" icon={<Send size={14} />}>
      ...
      {/* KPI grid - 2 cols on mobile for readability */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2.5 sm:gap-4 mb-4 sm:mb-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-[13px] sm:text-[14px] font-semibold text-[--color-ink] truncate">{r.name}</div>
                      <div className="text-[11px] text-[--color-muted] truncate">{r.source} · {r.time}</div>
                    </div>
                    <div className="text-[12px] sm:text-[13px] shrink-0 leading-none pt-0.5">{"⭐".repeat(r.rating)}</div>
                  </div>
                  <p className="text-[12.5px] sm:text-[13px] leading-relaxed text-[--color-body] mt-2 break-words">{r.text}</p>
